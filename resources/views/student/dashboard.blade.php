...
<body class="bg-light-bg-primary dark:bg-dark-bg-primary text-light-text-primary dark:text-dark-text-primary transition-colors duration-300">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
            <h2 class="text-lg font-semibold text-light-text-primary dark:text-dark-text-primary mb-4">Filter by Category</h2>
            <form method="GET" class="flex flex-wrap gap-3">
                ...
                @foreach($categories as $value => $label)
                    ...
                    <button type="submit" name="category" value="{{ $value }}" 
                        class="px-4 py-2 text-sm font-medium rounded-full border-2 transition-all duration-200 transform hover:scale-105
                        @if($isActive)
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <!-- Course Card -->
            <div class="bg-light-bg-secondary dark:bg-dark-bg-secondary rounded-xl shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 overflow-hidden border border-light-border-subtle dark:border-dark-border-subtle">
                <div class="h-48 relative">
                    ...
                    <div class="absolute bottom-4 left-4">
                        <span class="px-3 py-1 glass text-dark-text-primary bg-opacity-70 bg-dark-bg-secondary text-xs font-semibold rounded-full">
                            {{ $course->category->name ?? 'General' }}
                        </span>
                    </div>
                </div>
                <div class="p-6">
                    <h3 class="font-semibold text-lg text-light-text-primary dark:text-dark-text-primary mb-2">{{ $course->title }}</h3>
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center space-x-2">
                            ...
                            <span class="text-sm text-light-text-secondary dark:text-dark-text-secondary">{{ $course->coach->name }}</span>
                        </div>
                        <div class="flex items-center">
                            ...
                            <span class="text-light-text-muted dark:text-dark-text-muted text-sm ml-1">{{ number_format($rating, 1) }}</span>
                        </div>
                    </div>
                    <div class="flex items-center justify-between">
                        ...
                        <a href="/cours/{{ $course->id }}" class="px-6 py-2 bg-light-accent-secondary hover:bg-light-accent-secondary/90 dark:bg-dark-accent-secondary dark:hover:bg-dark-accent-secondary/90 text-dark-text-primary text-sm font-medium rounded-lg transition-colors duration-200 shadow-sm hover:shadow-md">
                            View Course
                        </a>
                    </div>
                </div>
            ...
            <div class="col-span-full text-center py-12">
                @if($selectedCategory)
                    <div class="flex flex-col items-center">
                        <svg class="w-16 h-16 text-light-text-muted dark:text-dark-text-muted mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                        <p class="text-light-text-muted dark:text-dark-text-muted text-lg mb-2">No courses found in "{{ ucfirst(str_replace('-', ' ', $selectedCategory)) }}" category.</p>
                        <a href="{{ route('student.dashboard') }}" class="mt-4 px-6 py-2 bg-light-accent-secondary hover:bg-light-accent-secondary/90 dark:bg-dark-accent-secondary dark:hover:bg-dark-accent-secondary/90 text-dark-text-primary text-sm font-medium rounded-lg transition-colors duration-200">
                            View All Courses
                        </a>
                    </div>
                @else
...
<div id="modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    ...
    <div class="bg-light-bg-secondary rounded-lg shadow-xl max-w-md w-full mx-4 transform transition-all">
        ...
        <div class="px-6 py-6 text-center">
            ...
            <p class="text-sm text-light-text-muted">Your enrollment has been confirmed and you now have access to the course.</p>
        </div>
        
        <!-- Modal Footer -->
        <div class="px-6 py-4 border-t border-light-border-default flex gap-3">
            <button onclick="closeModal()" class="flex-1 bg-green-600 text-dark-text-primary py-2 px-4 rounded-md hover:bg-green-700 transition-colors font-medium">
                Continue to Course
            </button>
            <button onclick="closeModal()" class="flex-1 bg-gray-200 text-light-text-primary py-2 px-4 rounded-md hover:bg-gray-300 transition-colors font-medium">
                Close
            </button>
        </div>
    </div>
...
@endif --}}
    ...
    <!-- Footer -->
    <footer class="bg-light-bg-secondary dark:bg-dark-bg-secondary border-t border-light-border-default dark:border-dark-border-default transition-colors duration-300 mt-16">
