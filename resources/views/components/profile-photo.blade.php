...
@if($dropdown)
    ...
    <div class="relative">
        <button onclick="toggleProfileDropdown()" class="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
            ...
            @else
                <div class="{{ $currentSize }} rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-white flex items-center justify-center font-bold shadow-sm hover:shadow-md transition-all duration-200 border-2 border-transparent hover:border-indigo-400 dark:hover:border-indigo-500">
...
@else
    ...
    @else
        <div class="{{ $currentSize }} rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-white flex items-center justify-center font-bold shadow-sm hover:shadow-md transition-all duration-200 border-2 border-transparent hover:border-indigo-400 dark:hover:border-indigo-500 {{ $attributes->get('class') }}">
            {{ $firstInitial }}{{ $lastInitial }}
        </div>
    @endif
    
    @if($showName && !$dropdown)
        <span class="ml-2 text-sm font-medium text-gray-700 dark:text-gray-300">{{ $user->name }}</span>
    @endif
@endif