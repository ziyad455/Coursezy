                           ...
                           class="block px-4 py-3 hover:bg-light-bg-tertiary dark:hover:bg-dark-bg-tertiary transition-colors duration-200 {{ $selectedUser && $selectedUser->id == $user->id ? 'bg-blue-50 dark:bg-blue-900/30 border-l-4 border-blue-500 dark:border-blue-400' : '' }}">
                            <div class="flex items-center space-x-3">
                                <div class="flex-shrink-0">
                                    <div class="w-10 h-10 rounded-full bg-gradient-to-r from-blue-400 to-purple-500 dark:from-blue-500 dark:to-purple-600 flex items-center justify-center text-dark-text-primary font-semibold">
                ...
                @if($selectedUser)
                    ...
                    <div class="px-6 py-4 bg-light-bg-secondary dark:bg-dark-bg-secondary border-b border-light-border-default dark:border-dark-border-default">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-light-text-primary dark:text-dark-text-primary">{{ $selectedUser->name }}</h3>
                                <p class="text-sm text-light-text-muted dark:text-dark-text-muted">{{ $selectedUser->role ?? 'User' }}</p>
                            </div>
                        </div>
                    </div>

                    <!-- Messages -->
                    <div id="messages-container" class="flex-1 overflow-y-auto p-6 space-y-4 bg-light-bg-primary dark:bg-dark-bg-primary">
                        @foreach($messages as $message)
                            <div class="flex {{ $message->from_user_id == Auth::id() ? 'justify-end' : 'justify-start' }}">
                                <div class="max-w-xs lg:max-w-md">
                                    <div class="{{ $message->from_user_id == Auth::id() ? 'bg-blue-500 dark:bg-blue-600 text-dark-text-primary' : 'bg-light-bg-secondary dark:bg-dark-bg-secondary text-light-text-primary dark:text-dark-text-primary' }} rounded-lg px-4 py-2 shadow dark:shadow-md">
                                        <p class="text-sm">{{ $message->message }}</p>
                                    </div>
                                    <p class="text-xs text-light-text-muted dark:text-dark-text-muted mt-1 {{ $message->from_user_id == Auth::id() ? 'text-right' : 'text-left' }}">
                ...
                @else
                    ...
                    <div class="flex-1 flex items-center justify-center bg-light-bg-primary dark:bg-dark-bg-primary">
                        <div class="text-center">
                            <svg class="mx-auto h-12 w-12 text-gray-400 dark:text-light-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path>
                            </svg>
                            <h3 class="mt-2 text-sm font-medium text-light-text-primary dark:text-dark-text-primary">No conversation selected</h3>
                            <p class="mt-1 text-sm text-light-text-muted dark:text-dark-text-muted">Choose a user from the list to start messaging</p>
                        </div>
                    </div>
                @endif
            </div>
