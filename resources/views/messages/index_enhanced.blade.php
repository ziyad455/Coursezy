...
<body class="bg-light-bg-primary dark:bg-dark-bg-primary text-light-text-primary dark:text-dark-text-primary transition-colors duration-300" 
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="bg-light-bg-secondary dark:bg-dark-bg-secondary rounded-lg shadow-lg dark:shadow-xl overflow-hidden" style="height: 80vh;">
            <div class="flex h-full">
                
                <!-- Conversations List -->
                <div class="w-1/3 border-r border-light-border-default dark:border-dark-border-default flex flex-col">
                    <!-- Header -->
                    <div class="p-4 bg-gradient-to-r from-blue-500 to-purple-600 dark:from-blue-600 dark:to-purple-700 text-dark-text-primary">
                    ...
                    <div class="flex-1 overflow-y-auto custom-scrollbar">
                            ...
                            <div @click="selectUser(conversation.user.id)" 
                                ...
                                <div class="flex items-start space-x-3">
                                    ...
                                    <div class="flex-shrink-0 relative">
                                        <template x-if="conversation.user.profile_photo && conversation.user.profile_photo.startsWith('https://')">
                                            <img :src="conversation.user.profile_photo" 
                                        ...
                                        <template x-if="!conversation.user.profile_photo">
                                            <div class="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-dark-text-primary font-semibold shadow-sm">
                                                <span x-text="conversation.user.name.charAt(0).toUpperCase() + (conversation.user.name.split(' ')[1] ? conversation.user.name.split(' ')[1].charAt(0).toUpperCase() : '')"></span>
                                            </div>
                                        </template>
                                        <!-- Online indicator -->
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center justify-between">
                                            <p class="text-sm font-semibold text-light-text-primary dark:text-dark-text-primary truncate" x-text="conversation.user.name"></p>
                                        ...
                                        <div class="flex items-center justify-between mt-1">
                                            <p class="text-sm text-light-text-secondary dark:text-dark-text-secondary truncate">
                                                <span x-show="conversation.last_message">
                                                    <span x-show="conversation.last_message.from_user_id === {{ Auth::id() }}" class="text-light-text-muted">You: </span>
                                                    <span x-text="conversation.last_message ? conversation.last_message.message : ''"></span>
                                                </span>
                                                <span x-show="!conversation.last_message" class="italic text-dark-text-secondary">No messages yet</span>
                                            </p>
                                        ...
                                        <!-- Typing indicator -->
                                        <div x-show="typingUsers[conversation.user.id]" class="flex items-center space-x-1 mt-1">
                                            <span class="text-xs text-light-text-muted dark:text-dark-text-muted italic">typing</span>
                        ...
                        <!-- Empty state -->
                        <div x-show="filteredConversations.length === 0" class="p-8 text-center text-light-text-muted dark:text-dark-text-muted">
                ...
                <div class="flex-1 flex flex-col" x-show="selectedUserId">
                    <!-- Chat Header -->
                    <div class="px-6 py-4 bg-light-bg-secondary dark:bg-dark-bg-secondary border-b border-light-border-default dark:border-dark-border-default">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-3">
                                <template x-if="selectedUser?.profile_photo && selectedUser.profile_photo.startsWith('https://')">
                                ...
                                <template x-if="selectedUser?.profile_photo && !selectedUser.profile_photo.startsWith('https://')">
                                    <img :src="`/storage/${selectedUser.profile_photo}`" 
                                         :alt="selectedUser.name" 
                                         class="w-10 h-10 rounded-full object-cover border-2 border-transparent hover:border-indigo-400 dark:hover:border-indigo-500 transition-colors">
                                </template>
                                <template x-if="!selectedUser?.profile_photo">
                                    <div class="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-dark-text-primary font-semibold shadow-sm">
                                        <span x-text="selectedUser?.name?.charAt(0).toUpperCase() + (selectedUser?.name?.split(' ')[1] ? selectedUser.name.split(' ')[1].charAt(0).toUpperCase() : '')"></span>
                                    </div>
                                </template>
                                <div>
                                    <h3 class="text-lg font-semibold text-light-text-primary dark:text-dark-text-primary" x-text="selectedUser?.name"></h3>
                                    <p class="text-sm text-light-text-muted dark:text-dark-text-muted">
                                        <span x-show="isUserOnline(selectedUserId)" class="text-green-500">● Online</span>
                                        <span x-show="!isUserOnline(selectedUserId)">Last seen <span x-text="selectedUser?.last_seen"></span></span>
                                    </p>
                                </div>
                            </div>
                    ...
                    <div class="flex-1 overflow-y-auto p-6 space-y-4 bg-light-bg-primary dark:bg-dark-bg-primary custom-scrollbar" 
                            ...
                            <div :class="message.from_user_id === {{ Auth::id() }} ? 'justify-end' : 'justify-start'" 
                                ...
                                <div class="max-w-xs lg:max-w-md">
                                    <div :class="message.from_user_id === {{ Auth::id() }} ? 
                                        ...
                                        <button x-show="message.from_user_id === {{ Auth::id() }}" @click="deleteMessage(message)" class="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center opacity-0 group-hover:opacity-100 transition" title="Delete">
                                            ×
                                        </button>
                                    </div>
                                    <div class="flex items-center mt-1 space-x-2" 
                                         :class="message.from_user_id === {{ Auth::id() }} ? 'justify-end' : 'justify-start'">
                                        <p class="text-xs text-light-text-muted dark:text-dark-text-muted" x-text="formatTime(message.created_at)"></p>
                        ...
                        <div x-show="typingUsers[selectedUserId]" class="flex justify-start">
                            <div class="bg-light-bg-secondary dark:bg-dark-bg-secondary rounded-lg px-4 py-2 shadow dark:shadow-md">
                    ...
                    <!-- Message Input -->
                    <div class="px-6 py-4 bg-light-bg-secondary dark:bg-dark-bg-secondary border-t border-light-border-default dark:border-dark-border-default">
                ...
                <div class="flex-1 flex items-center justify-center bg-light-bg-primary dark:bg-dark-bg-primary" x-show="!selectedUserId">
                    <div class="text-center">
                        <svg class="mx-auto h-16 w-16 text-gray-400 dark:text-light-text-muted mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path>
                        </svg>
                        <h3 class="text-lg font-medium text-light-text-primary dark:text-dark-text-primary mb-2">Welcome to Coursezy Messages</h3>
                        <p class="text-sm text-light-text-muted dark:text-dark-text-muted">Select a conversation from the left to start messaging</p>
                    </div>
                </div>
            </div>
        </div>
