...
<x-coachNav/>
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <form action="/accont" method="POST" enctype="multipart/form-data" class="space-y-8">
            ...
            <!-- Info Banner -->
            <div class="bg-indigo-50 dark:bg-indigo-900/20 border-l-4 border-indigo-500 p-4 rounded-r-lg opacity-0 animate-fade-in">
                <div class="flex items-start">
                    <svg class="w-6 h-6 text-indigo-600 dark:text-indigo-400 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-indigo-700 dark:text-indigo-400">
                            All fields below show your current saved information. Make any changes you'd like and click "Save Profile" when you're done.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-light-bg-secondary dark:bg-dark-bg-secondary rounded-xl shadow-md border border-light-border-subtle dark:border-dark-border-subtle p-6 opacity-0 animate-fade-in stagger-1">
                <h2 class="text-xl font-semibold text-light-text-primary dark:text-dark-text-primary mb-6">Profile Picture</h2>
                ...
                <div class="flex flex-col items-center">
                    ...
                    <div class="relative mb-6">
                        <div class="profile-image-container w-40 h-40 cursor-pointer relative group" onclick="document.getElementById('profile-picture').click()" role="button" tabindex="0" aria-label="Change profile picture">
                            <!-- Decorative Ring -->
                            <div class="absolute inset-0 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-500 opacity-20 group-hover:opacity-30 transition-opacity duration-300 pointer-events-none"></div>
                            <div class="absolute inset-1 rounded-full bg-light-bg-secondary dark:bg-dark-bg-secondary pointer-events-none"></div>
                            
                            <!-- Image Container - Current Photo Displayed Here -->
                            <div class="absolute inset-2 rounded-full overflow-hidden">
                                <div id="profile-photo-display" class="w-full h-full flex items-center justify-center">
                                    @if($user->profile_photo)
                                        @if(Str::startsWith($user->profile_photo, 'https://'))
                                            <img src="{{ $user->profile_photo }}" alt="{{ $user->name }}" class="w-full h-full object-cover rounded-full">
                                        @else
                                    ...
                                    @else
                                        ...
                                        @endphp
                                        <div class="w-full h-full rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-white flex items-center justify-center font-bold text-3xl">
                                            {{ $firstInitial }}{{ $lastInitial }}
                                        </div>
                                    @endif
                                </div>
                                <img id="profile-preview" src="" alt="Profile Picture" loading="lazy" class="w-full h-full object-cover rounded-full hidden">
                            </div>
                            
                            <!-- Hover Overlay -->
                            <div class="upload-overlay rounded-full">
                                <div class="flex flex-col items-center">
                                    <svg class="w-10 h-10 text-white mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <div class="bg-light-bg-secondary dark:bg-dark-bg-secondary rounded-xl shadow-md border border-light-border-subtle dark:border-dark-border-subtle p-6 opacity-0 animate-fade-in stagger-2">
                <h2 class="text-xl font-semibold text-light-text-primary dark:text-dark-text-primary mb-6">Personal Information</h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-light-text-muted dark:text-dark-text-muted mt-1">Email cannot be changed from this page</p>
                    </div>
                </div>
            </div>

            <!-- About Section -->
            <div class="bg-light-bg-secondary dark:bg-dark-bg-secondary rounded-xl shadow-md border border-light-border-subtle dark:border-dark-border-subtle p-6 opacity-0 animate-fade-in stagger-3">
                <h2 class="text-xl font-semibold text-light-text-primary dark:text-dark-text-primary mb-6">About</h2>
                
                <!-- About -->
                <div class="mb-6">
                    <label for="about" class="block text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary mb-2">
            ...
            <div class="bg-light-bg-secondary dark:bg-dark-bg-secondary rounded-xl shadow-md border border-light-border-subtle dark:border-dark-border-subtle p-6 opacity-0 animate-fade-in stagger-4">
                <h2 class="text-xl font-semibold text-light-text-primary dark:text-dark-text-primary mb-6">Professional Information</h2>
                ...
                <div>
                    ...
                    <div class="flex mb-4">
                        <input type="text" id="skill-input" placeholder="Add a skill (e.g., JavaScript, Leadership, etc.)"
                               class="editable-field-hint flex-1 px-4 py-3 border border-light-border-default dark:border-dark-border-default rounded-l-lg focus:ring-2 focus:ring-light-accent-secondary dark:focus:ring-dark-accent-secondary dark:focus:ring-indigo-400 focus:border-transparent bg-white dark:bg-gray-700 text-light-text-primary dark:text-dark-text-primary transition-all duration-200">
                        <button type="button" onclick="addSkill()" 
                                class="px-6 py-3 bg-light-accent-secondary hover:bg-light-accent-secondary/90 text-dark-text-primary font-medium rounded-r-lg transition-colors">
                            Add
                        </button>
                    </div>

                    <!-- Skills Display - loaded from database -->
                    <div id="skills-container" class="flex flex-wrap gap-2 min-h-[50px] p-3 border-2 border-dashed border-light-border-default dark:border-dark-border-default rounded-lg">
                        ...
                        @empty
                            <p class="text-light-text-muted dark:text-dark-text-muted text-sm">No skills added yet. Add your first skill above.</p>
                        @endforelse
                    </div>
    ...
    <script>
        ...
        function removeSkill(skill) {
            ...
            if (skillsContainer.children.length === 0) {
                skillsContainer.innerHTML = '<p class="text-light-text-muted dark:text-dark-text-muted text-sm">No skills added yet. Add your first skill above.</p>';
            }
