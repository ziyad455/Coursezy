...
<head>
    ...
    <title>Course Content - Coursezy</title>
    ...
    <main class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-28">
        ...
        <div class="mb-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                <div>
                    ...
                    <p class="text-light-text-secondary dark:text-dark-text-secondary flex items-center gap-2">
                        <span
                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-light-accent-secondary/10 dark:bg-dark-accent-secondary/20 text-light-accent-secondary dark:text-dark-accent-secondary">
                            Step 2 of 2
                        </span>
            ...
            <div class="flex flex-wrap gap-3">
                ...
                <div
                    class="flex items-center gap-2 px-4 py-2 rounded-lg bg-light-bg-secondary dark:bg-dark-bg-secondary border border-light-border-subtle dark:border-dark-border-subtle">
                    <div
                        class="w-8 h-8 rounded-lg bg-light-accent-success/10 dark:bg-dark-accent-success/20 flex items-center justify-center">
                        <svg class="w-4 h-4 text-light-accent-success dark:text-dark-accent-success" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div>
                        ...
                        <p class="text-sm font-semibold text-light-text-primary dark:text-dark-text-primary"
                            x-text="getUploadedVideosCount()"></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <form id="sectionsForm" @submit.prevent="submitForm()"
            ...
            <div id="sectionsContainer" class="space-y-6">
                <template x-for="(section, sectionIndex) in sections" :key="section.id">
                    <div class="section-card" x-transition:enter="transition ease-out duration-500"
                        ...
                        <div
                            class="bg-light-bg-secondary dark:bg-dark-bg-secondary rounded-xl shadow-md border border-light-border-subtle dark:border-dark-border-subtle overflow-hidden">
                            <!-- Section Header -->
                            <div
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h3
                                                ...
                                                Section <span x-text="sectionIndex + 1"></span></h3>
                                            <p class="text-xs text-light-text-muted dark:text-dark-text-muted"
                                                x-text="`${section.videos.length} video${section.videos.length !== 1 ? 's' : ''}`">
                                            </p>
                                        </div>
                                    </div>
                            ...
                            <div class="p-6 space-y-6">
                                ...
                                <div class="space-y-4">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <span
                                            class="text-xs px-2 py-1 rounded-full bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 font-medium"
                                    ...
                                    <div class="space-y-3">
                                        <template x-for="(video, videoIndex) in section.videos" :key="video.id">
                                            <div class="video-card relative rounded-lg overflow-hidden" :class="{
                                                ...
                                                <div x-show="video.status === 'success'"
                                                    class="success-pulse absolute inset-0 rounded-lg pointer-events-none"
                                                    x-transition></div>

                                                <div class="relative border rounded-lg p-4" :class="{
                                                    ...
                                                    <div class="flex items-start justify-between mb-4">
                                                        <div class="flex items-center gap-3">
                                                            <div class="w-9 h-9 rounded-lg flex items-center justify-center text-sm font-bold"
                                                                ...
                                                                <template
                                                                    x-if="video.status === 'idle' && !video.uploading">
                                                            ...
                                                            <div>
                                                                <p
                                                                    ...
                                                                    Video <span x-text="videoIndex + 1"></span></p>
                                                                <p class="text-xs" :class="{
                                                                    ...
                                                                    x-text="video.uploading ? 'Uploading...' : (video.status === 'success' ? 'Ready' : (video.status === 'error' ? 'Failed' : 'Pending'))">
                                                                </p>
                                                            </div>
                                                        </div>
                                                        <button type="button"
                                                            @click="removeVideo(sectionIndex, videoIndex)"
                                                    ...
                                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                        ...
                                                        <div>
                                                            <label
                                                                class="block text-xs font-medium text-light-text-muted dark:text-dark-text-muted mb-1.5">Video
                                                                Title</label>
                                                            <input type="text"
                                                    ...
                                                    <div x-show="video.uploading" class="mt-4" x-transition>
                                                        <div class="flex justify-between text-xs font-medium mb-2">
                                                            <span
                                                                class="text-light-accent-secondary dark:text-dark-accent-secondary flex items-center gap-1">
                                                            ...
                                                            <span
                                                                class="text-light-text-secondary dark:text-dark-text-secondary font-bold"
                                                                x-text="`${video.progress}%`"></span>
                                                        </div>
                                                        <div
                                                    ...
                                                    <!-- Status Messages -->
                                                    <div x-show="video.status === 'success'"
                                                        class="mt-3 flex items-center gap-2 text-sm text-light-accent-success dark:text-dark-accent-success"
                                                        x-transition>
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                            viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                    ...
                                                    <div x-show="video.status === 'error'"
                                                        class="mt-3 flex items-center gap-2 text-sm text-light-accent-error dark:text-dark-accent-error"
                                                        x-transition>
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                            viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                    ...
                                                    <input type="hidden"
                                                        :name="`sections[${sectionIndex}][videos][${videoIndex}][video_url]`"
                                                        x-model="video.video_url">
                                                    <input type="hidden"
                                                        :name="`sections[${sectionIndex}][videos][${videoIndex}][public_id]`"
                                                        x-model="video.public_id">
                                                    <input type="hidden"
                                                        :name="`sections[${sectionIndex}][videos][${videoIndex}][original_name]`"
                                                        x-model="video.original_name">
                                                    <input type="hidden"
                                                        :name="`sections[${sectionIndex}][videos][${videoIndex}][size]`"
                                                        x-model="video.size">
                                                    <input type="hidden"
                                                        :name="`sections[${sectionIndex}][videos][${videoIndex}][mime_type]`"
                                                        x-model="video.mime_type">
                                                </div>
                                            </div>
                                        </template>
                                    </div>
            ...
            <div x-show="sections.length === 0" x-transition class="text-center py-16">
                ...
                <h3 class="text-xl font-semibold text-light-text-primary dark:text-dark-text-primary mb-2">No sections
                    yet</h3>
                <p class="text-light-text-muted dark:text-dark-text-muted mb-6 max-w-sm mx-auto">Start building your
                    course by adding your first section with videos.</p>
            ...
            <!-- Bottom Action Bar -->
            <div
                class="fixed bottom-0 left-0 right-0 z-40 bg-light-bg-secondary/95 dark:bg-dark-bg-secondary/95 backdrop-blur-sm border-t border-light-border-subtle dark:border-dark-border-subtle shadow-lg">
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div x-show="isUploading"
                                class="flex items-center gap-2 text-sm text-light-accent-secondary dark:text-dark-accent-secondary">
