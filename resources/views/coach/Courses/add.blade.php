...
<body
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <!-- Course Form -->
        <div
            class="bg-light-bg-secondary dark:bg-dark-bg-secondary rounded-xl shadow-md border border-light-border-subtle dark:border-dark-border-subtle p-8">
            <form id="courseForm" class="space-y-6" action="{{ route('coach.courses.store') }}" method="POST"
                enctype="multipart/form-data">
                @csrf
                <!-- Course Title -->
                <div>
                    <label for="title"
                        class="block text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary mb-2">Course
                        Title *</label>
                ...
                <div>
                    <label for="description"
                        class="block text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary mb-2">Description
                        *</label>
                ...
                <div>
                    <label for="category"
                        class="block text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary mb-2">Category
                        *</label>
                    <select id="category_id" name="category_id" required
                        ...
                        <option value="10" {{ old('category_id') == '10' ? 'selected' : '' }}>Personal Development
                        </option>
                        <option value="11" {{ old('category_id') == '11' ? 'selected' : '' }}>Artificial Intelligence
                        </option>
                    </select>
                    @error('category_id')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                ...
                <div>
                    <label
                        ...
                        Thumbnail *</label>
                    <div id="upload-area"
                        class="relative border-2 border-light-border-default dark:border-dark-border-default border-dashed rounded-lg p-6 text-center hover:border-indigo-500 transition-colors cursor-pointer">
                        <input type="file" id="thumbnail" name="thumbnail" accept="image/*" class="hidden">
                        <div id="upload-content">
                            <svg class="mx-auto h-12 w-12 text-dark-text-secondary" stroke="currentColor" fill="none"
                                viewBox="0 0 48 48">
                                <path
                            ...
                            <p class="mt-2 text-sm text-light-text-secondary dark:text-dark-text-secondary">
                                <span class="font-medium text-indigo-600 dark:text-indigo-400">Click to upload</span> or
                                drag and drop
                            </p>
                        ...
                        <div id="image-preview" class="hidden">
                            ...
                            <p id="file-name"
                                class="mt-2 text-sm text-light-text-secondary dark:text-dark-text-secondary"></p>
                            <button type="button" id="remove-image"
                                class="mt-2 text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 text-sm">Remove</button>
                        </div>
                    </div>
                    @error('thumbnail')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                ...
                <div>
                    <label for="price"
                        class="block text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary mb-2">Price
                        ($) *</label>
                    <div class="relative">
                ...
                <div>
                    <label
                        class="block text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary mb-3">Course
                        Status</label>
                    <div class="grid grid-cols-2 gap-4">
                        <label
                            class="flex items-center p-4 border border-light-border-default dark:border-dark-border-default rounded-lg cursor-pointer hover:bg-light-bg-tertiary dark:hover:bg-dark-bg-tertiary transition-colors">
                            <input type="radio" name="status" value="draft" {{ old('status') == 'draft' ? 'checked' : '' }}
                                class="h-4 w-4 text-indigo-600 focus:ring-light-accent-secondary dark:focus:ring-dark-accent-secondary">
                            <div class="ml-3">
                                <span
                                    class="block text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary">Draft</span>
                                <span class="block text-xs text-light-text-muted dark:text-dark-text-muted">Save for
                                    later</span>
                            </div>
                        </label>
                        <label
                            ...
                            <div class="ml-3">
                                <span
                                    class="block text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary">Published</span>
                                <span class="block text-xs text-light-text-muted dark:text-dark-text-muted">Make it
                                    live</span>
                            </div>
                        </label>
                    </div>
                ...
                <!-- Form Actions -->
                <div
                    class="flex justify-end space-x-4 pt-6 border-t border-light-border-default dark:border-dark-border-default">
